...
export default function PolicyPage({ title, content, onBack }: { title: string; content: string[]; onBack: () => void }) {
  return <section className="glass-panel rounded-[2rem] p-6 sm:p-10">
    <button type="button" onClick={onBack} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200"><ArrowLeft className="h-4 w-4" /> Back to scanner</button>
